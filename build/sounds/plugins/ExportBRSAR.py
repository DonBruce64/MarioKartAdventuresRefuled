...
def exportRBNK(bankNode, filePrefix, filePrefixFunction):
	bankEntry = 0;
	count = 0;
			...
			for soundChildNode in soundChild.Children:
				if type(soundChildNode) == RWAVNode:
					...
					if fileName is not None:
						soundChildNode.Export(fileName)
						count += 1
					bankEntry += 1
	return count
...
if BrawlAPI.RootNode != None: # If there is a valid open file
	...
	if folder:
		...
		#First export the main VO sounds that we can replace from the tree.
		soundNodes = BrawlAPI.NodeListOfType[RSARSoundNode]()
		for soundNode in soundNodes:
			if soundNode.CreateStreams()[0] is not None:
				if canEditVO(soundNode.TreePath):
					soundNode.Export(folder + "/" + soundNode.TreePath.replace("/", "_") + ".wav")
					count += 1
		...
		for soundFile in BrawlAPI.RootNode.Files:
			if type(soundFile) == RBNKNode and canEditVO(soundFile.Name):
				#Export a VO bank.  These will be the STRM (slipstream) files.
				...
				count += exportRBNK(soundFile, filePrefix, genStreamPrefix)
			if soundFile.FileNodeIndex == 637:
				#Export the GRP char select voices.  These reference an external list for common import/export operations.
				count += exportRBNK(soundFile, folder + "/VO_", genKartSelectPrefix)
				
		if count: # If any sounds are found, show the user a success message
			BrawlAPI.ShowMessage(str(count) + " sounds were successfully exported to " + folder, "Success")
